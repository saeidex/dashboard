  ...
  responses: {
    ...
    [HttpStatusCodes.UNPROCESSABLE_ENTITY]: jsonContent(
      createErrorSchema(patchOrdersSchema)
        .or(createErrorSchema(OrderIdParamsSchema)),
      "The validation error(s)",
    ),
  },
});
...
const OrderItemIdParamsSchema = z.object({
  id: z.string().min(1).openapi({ example: "b3c0f3f2-2b8a-4d9d-9c1a-123456789abc" }),
  itemId: z.string().min(1).openapi({ example: "9f7a1e34-6cd2-4a77-b9e1-abcdef012345" }),
});
...
export const createItem = createRoute({
  ...
  request: {
    params: OrderIdParamsSchema,
    body: jsonContentRequired(insertOrderItemsSchema, "The order item to create"),
  },
  tags: itemTags,
  responses: {
    [HttpStatusCodes.OK]: jsonContent(
  ...
  responses: {
    ...
    [HttpStatusCodes.UNPROCESSABLE_ENTITY]: jsonContent(
      createErrorSchema(patchOrderItemsSchema)
        .or(createErrorSchema(orderItemIdParam)),
      "Validation error(s)",
    ),
  },
});
