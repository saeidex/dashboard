...
export const list: AppRouteHandler<ListRoute> = async (c) => {
  ...
  const statuses = query.status && query.status.length > 0 ? query.status : undefined;
  const categoryIds = query.categoryId && query.categoryId.length > 0 ? query.categoryId : undefined;
