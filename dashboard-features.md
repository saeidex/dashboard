...
System / Administrative (users, roles activity) – optional or separate admin tab.
3. Executive Overview (Hero Section)
...
Period selector applying to entire board via context.
4. Sales & Revenue Section
Metrics:
...
Trend windows (rolling 7-day average)
5. Orders Pipeline
Metrics:
...
Aggregate orderItems by productId: sum(quantity), sum(pricing.total).
...
Category Concentration Index (Herfindahl-Hirschman: sum(share^2)) – optional advanced insight.
8. Expenses & Profitability
Metrics:
...
Align time granularity to orders (daily).
9. Vendors & Supply
Metrics:
...
No schema linking vendors to products or expenses—flag as future enhancement.
10. Employees & Workforce
Metrics:
...
Attrition Rate (needs termination dates—recommend adding terminatedAt).
11. Tasks & Operational Flow
Metrics:
...
Add createdAt / completedAt to enable throughput metrics (cycle time, WIP).
12. Users / Admin
Metrics:
...
Table: Recently Added Users.
13. Cross-Domain Composite Insights
...
Discount Impact: DiscountTotal / (ItemsTotal + DiscountTotal).
14. Recommended Additions (Schema Gaps)
...
Inventory Movements: separate table (productId, delta, reason, createdAt) to compute turnover.
15. Visualization Mapping Table (Summary)
KPI Cards:
...
Employee Position Bar → Filter hires table.
18. Data Aggregation Layer (Suggested)
...
employee_position_agg: position, count_active. Caching: Recompute nightly + incremental updates on write events.
...
On employee status change: mark current active count metric cache invalid.
...
Combine small KPIs into a single batched endpoint (/dashboard/summary).
21. API Endpoint Sketch (If building backend later)
...
Introduce missing schema fields (cost, timestamps), inventory movements, vendor spend integration.
23. Data Quality & Validation Notes
